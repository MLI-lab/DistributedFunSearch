...
Measures where time is spent during priority function evaluation:
- Graph loading time
- Priority computation (tests O(1), O(n), O(nodes) complexity)
- Sorting time
- Greedy selection time
- CPU utilization per phase

Profiles both graph-based (NetworkX) and no-graph evaluation modes.
