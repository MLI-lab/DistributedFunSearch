...
Usage:
    python run_full_analysis.py <checkpoint_paths...> [options]

Examples:
    # Single checkpoint file
    python run_full_analysis.py checkpoint.pkl --output ./my_analysis/

    # Single folder (scans all .pkl files recursively)
    python run_full_analysis.py /mnt/disfun/checkpoints/checkpoint_run_xxx/

    # Multiple folders (combine experiments)
    python run_full_analysis.py /exp1/checkpoints/ /exp2/checkpoints/ --output ./combined/

    # Mix of files and folders
    python run_full_analysis.py /exp1/ /exp2/latest.pkl --max-n 14

    # Use only latest checkpoint from each folder
    python run_full_analysis.py /exp1/ /exp2/ /exp3/ --latest-only
"""
...
def main():
    ...
    if not args.skip_extract:
        cmd = [
            sys.executable, str(script_dir / "extract_successful_functions.py"),
        ]
    ...
    if not args.skip_eval:
        cmd = [
            sys.executable, str(script_dir / "evaluate_extended.py"),
            str(functions_json),
    ...
    cmd = [
        sys.executable, str(script_dir / "vt_overlap_analysis.py"),
        str(codebooks_json),
        "--output", str(output_dir),
        "--vt-a", str(args.vt_a),
    ]
