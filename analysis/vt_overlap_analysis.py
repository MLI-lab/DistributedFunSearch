...
Usage:
    python vt_overlap_analysis.py <codebooks_json> [options]

Examples:
    python vt_overlap_analysis.py ./successful_functions/codebooks.json
    python vt_overlap_analysis.py ./codebooks.json --vt-a 1
    python vt_overlap_analysis.py ./codebooks.json --vt-path ../src/graphs/vt_solutions.json
"""
...
def group_functions_by_overlap(analyses: List[Dict], n_values: List[int]) -> Dict:
    ...
    3. perfect_mixed_all: 100% match (either VT or complement) for all n (no missing)
    ...
    for analysis in analyses:
        ...
        if missing:
            ...
            groups['perfect_mixed_all'].append(func_info)
...
def main():
    ...
    print("-" * 60)
    for group_name, func_list in groups.items():
        print(f"  {group_name:25s}: {len(func_list):4d} functions")
    print()
