        ...
        try:
            ...
            try:
                ...
                database = programs_database.ProgramsDatabase(
                    ...
                    start_n=self.config.evaluator.start_n, end_n=self.config.evaluator.end_n, s_values=self.config.evaluator.s_values, no_deduplication=self.config.programs_database.no_deduplication, prompt_limit=self.config.termination.prompt_limit if hasattr(self.config, 'termination') and self.config.termination else 400_000_000, optimal_solution_programs=self.config.termination.optimal_solution_programs if hasattr(self.config, 'termination') and self.config.termination else 200000, max_drain_time=self.config.termination.max_drain_time if hasattr(self.config, 'termination') and self.config.termination and hasattr(self.config.termination, 'max_drain_time') else 600, target_signatures=self.target_signatures,
