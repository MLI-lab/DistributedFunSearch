...
def compute_wandb_metrics(database) -> dict:
    ...
    # 8. Prompt statistics
    metrics["prompts/total"] = database.total_prompts
    ...
    # 11. Evolutionary lineage tracking (only if enabled)
                ...
                if database.wandb_enabled and database._wandb_initialized:
                    try:
                        artifact = wandb.Artifact(
                            name=f"lineage_island{island_id}_step{database.total_prompts}",
                        ...
                        lineage_link = f"See artifact: lineage_island{island_id}_step{database.total_prompts}"
...
async def periodic_wandb_logging(database):
