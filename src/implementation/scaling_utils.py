...
class ResourceManager:
    ...
    def start_sampler_process(self, target_function, args, processes, process_name, assignment):
        """Starts a sampler process with GPU assignment."""
        host_gpu, container_device = assignment
        proc = mp.Process(target=sampler_process_entry, args=(target_function, args, container_device), name=f"{process_name}-{len(processes)}")
        proc.start()
        processes.append(proc)
